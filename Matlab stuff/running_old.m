...
% for now, does un-normalized (need to uncomment normalizing on line 32)
% without the 64 subclusters
...
name_num = name_num(2);
CM = cool(name_num); % # of unique colors for that # of speakers

 for i = 1:size(pcaDataCell,2) %Plot averages for each speaker
    c = pcaDataCell{3,i};
    plot3(c(:,1),c(:,2),c(:,3),'*', 'color', CM(i,:));
 end
